  ...
  <meta charset="utf-8">
   ...
   $(function(){
     ...
     function change(check){
         if(check.checked){//체크했을경우
        	 
        	let aa = document.querySelectorAll("#ch");//aa는 배열, querySelectorAll은 ch인것 다 가져옴
        	for(let i=0; i<aa.length; i++){
        		aa[i].checked=true;//체크박스 ㄷㅏ 체크해라
        	}
         }else{//체크해제했을경우
     ...
     function order(){
    	 let cno = document.querySelectorAll("#ch");//cno,qty,size는 배열
    	 let qty = document.querySelectorAll("#qty");
    	 let size = document.querySelectorAll("#size");
    	 
    	 let cnt = 0;//체크값을 검사하는 변수
    	 let param_cno = '';//상품번호 여러개 연결
    	 let param_qty = '';//수량 여러개 연결
    	 let param_size ='';//사이즈 여러개 연결
    	 
    	 for(let i=0; i<cno.length; i++){
    		 if(cno[i].checked==true){//체크되어있다면
    			cnt++;
    		 	param_cno += cno[i].value + ',';
    		 	param_qty += qty[i].value + ',';
    		 	param_size += size[i].innerText+',' //인풋태그가 아니라서 value값못씀
    		 }
    	 }
